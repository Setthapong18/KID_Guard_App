...
class _ChildPinScreenState extends State<ChildPinScreen>
  ...
  String _errorText = 'PIN ไม่ถูกต้อง';
  ...
  void _onKeyPressed(String value) {
    // บล็อก input ถ้าถูกล็อกจากการลองผิดหลายครั้ง
    final auth = Provider.of<AuthProvider>(context, listen: false);
    if (auth.isPinLockedOut) {
      setState(() {
        _hasError = true;
        _errorText =
            'ลองผิดหลายครั้ง รอ ${auth.pinLockoutMinutesRemaining} นาที';
      });
  ...
  Future<void> _submit() async {
    ...
    if (success && mounted) {
      // บันทึก PIN และ UID ใน Secure Storage (Android Keystore)
      // ไม่ใช้ SharedPreferences เพราะ PIN เป็นข้อมูลสำคัญมาก
      await SecureStorageService.saveParentPin(_pin);
      if (auth.userModel != null) {
        await SecureStorageService.saveParentUid(auth.userModel!.uid);
      }
      ...
      // แสดง error จาก provider (มีจำนวนครั้งที่เหลือ / ล็อก)
      setState(() {
        _hasError = true;
        _errorText = auth.errorMessage ?? 'PIN ไม่ถูกต้อง';
      });
      HapticFeedback.heavyImpact();
