...
class _SelectUserScreenState extends State<SelectUserScreen>
  ...
  Future<void> _checkAuthState() async {
    ...
    // FIRST: Check if child mode is active (app relaunched after swipe-away or reboot)
    final prefs = await SharedPreferences.getInstance();
    final isChildModeActive = prefs.getBool('isChildModeActive') ?? false;
    // อ่านข้อมูล sensitive จาก Secure Storage
    final activeChildId = await SecureStorageService.getActiveChildId();
    final activeParentUid = await SecureStorageService.getParentUid();
    ...
    if (isChildModeActive && activeChildId != null && activeParentUid != null) {
      ...
      // ลอง restore จาก Firestore local cache ก่อน — ไม่ต้องต่อเน็ต
      // ดีสำหรับกรณีรีบูตเครื่อง/ปัดแอปทิ้ง ที่เน็ตยังไม่มาหรือช้า
      final offlineSuccess = await authProvider.restoreChildSessionOffline(
        parentUid: activeParentUid,
        childId: activeChildId,
      );

      if (offlineSuccess) {
        // restore สำเร็จ — เข้า Child Mode ทันที ไม่ต้องถามอะไรอีก
        if (mounted && !_hasNavigated) {
          _hasNavigated = true;
      ...
      // Offline restore ล้มเหลว (Firestore cache หมด) — ลอง login ผ่านเน็ต
      final savedPin = await SecureStorageService.getParentPin();
      if (savedPin != null && savedPin.isNotEmpty) {
        final netSuccess = await authProvider.childLogin(savedPin);
        if (netSuccess && authProvider.children.isNotEmpty) {
          try {
            final child = authProvider.children.firstWhere(
              (c) => c.id == activeChildId,
            );
            final selSuccess = await authProvider.selectChild(child);
            if (selSuccess) {
              if (mounted && !_hasNavigated) {
                _hasNavigated = true;
                Navigator.pushReplacementNamed(context, AppRoutes.childHome);
            ...
            // selectChild ล้มเหลว (เด็กถูกลบจาก parent จริงๆ) — clear session
            await prefs.setBool('isChildModeActive', false);
            await SecureStorageService.clearSession();
          } catch (_) {
            // childId ไม่เจอใน list (ถูกลบ) — clear session
            await prefs.setBool('isChildModeActive', false);
            await SecureStorageService.clearSession();
          }
        }
        // ถ้า netSuccess == false เพราะเน็ตล้มเหลว — ไม่ clearSession
        // เพื่อให้ลองใหม่ได้ตอนเปิดแอปครั้งถัดไป
      }
      ...
      if (savedPin != null && savedPin.isNotEmpty) {
        ...
        if (currentUser != null) {
          ...
          if (sessionDoc.exists) {
            final expiresAt = sessionDoc.data()?['expiresAt'] as Timestamp?;
            if (expiresAt != null &&
                DateTime.now().isAfter(expiresAt.toDate())) {
              // Session หมดอายุ — clear ทั้งหมด
