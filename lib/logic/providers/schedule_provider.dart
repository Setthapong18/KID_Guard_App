...
class ScheduleProvider extends ChangeNotifier {
  ...
  String? _errorMessage; // ✕ Error state เพื่อให้ UI แสดง error ได้
  ...
  Future<void> loadSchedules(String parentId, String childId) async {
    ...
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        if (data != null) {
          final List<SchedulePeriod> loadedPeriods = [];
      ...
      _errorMessage = 'ไม่สามารถโหลดตารางเวลาได้: ${e.message}';
    } catch (e) {
  ...
  /// Returns true ถ้าสำเร็จ, false ถ้าล้มเหลว (บอกสถานะให้ caller ผู้เรียก)
  Future<bool> saveSchedules(String parentId, String childId) async {
    ...
    try {
      ...
      _errorMessage = 'ไม่สามารถบันทึกตารางเวลาได้: ${e.message}';
      notifyListeners();
      return false;
    } catch (e) {
      _errorMessage = 'เกิดข้อผิดพลาด: $e';
