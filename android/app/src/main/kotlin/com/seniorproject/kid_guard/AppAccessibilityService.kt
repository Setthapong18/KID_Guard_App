...
class AppAccessibilityService : AccessibilityService() {
    ...
    private var lastBlockedHomeTime: Long = 0  // Debounce for blocked app home action
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (event?.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            ...
            if (isAppBlocked(packageName)) {
                // Debounce: skip if we just sent HOME within 2 seconds
                val now = System.currentTimeMillis()
                if (now - lastBlockedHomeTime < 2000) return
                ...
                // Go home immediately - no overlay to avoid flicker
                lastBlockedHomeTime = now
    ...
    private fun showOverlay(reason: String) {
        try {
            // Never show overlay for blocked apps - just go home
            if (currentRestrictionType == RestrictionType.BLOCKED_APP) {
                performGlobalAction(GLOBAL_ACTION_HOME)
                return
            }
